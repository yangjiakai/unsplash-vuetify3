...
<template>
  <v-card class="mt-2 my-favorite" height="80vh">
    ...
    <div v-if="unsplashStore.favoriteList.length > 0" class="favorite-container pa-3">
      <transition-group name="fade" class="">
        ...
        <v-card v-for="item in unsplashStore.favoriteList" :key="item.id"
          class="d-flex align-center favorite-card mb-3">
          <div style="width: 100px">
            <v-img :aspect-ratio="1" width="100" cover :src="item.urls.thumb" :lazy-src="item.urls.thumb" />
          </div>
