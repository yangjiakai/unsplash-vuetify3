      ...
      <v-col cols="12" xl="10">
        <v-card class="mt-2">
          <v-tabs v-model="tab" bg-color="primary">
            <v-tab value="photos"><v-icon class="mr-2">mdi-image-outline</v-icon> photos
              <span class="ml-2">({{ photoData.total }})</span></v-tab>
            <v-tab value="collections">
              collections<span class="ml-2">({{ collectionData.total }})</span></v-tab>
            <v-tab value="users"><v-icon class="mr-2">mdi-account-multiple</v-icon> users<span class="ml-2">({{
              userData.total
            }})</span></v-tab>
          </v-tabs>
          ...
          <v-card-text>
            <v-window v-model="tab">
                  ...
                  <v-row>
                    <v-col cols="12" xl="2" lg="3" md="4" sm="6" v-for="item in photoData.photos" :key="item.id">
                      <v-card width="100%" class="info-card photo-card d-flex flex-column" height="480">
                        <div>
                          <v-img class="align-end text-white" :src="item.urls.small" :lazy-src="item.urls.small"
                            height="300" cover @click="openPhotoDialog(item.id)">
                        ...
                        <v-card-actions>
                          <v-btn @click="toggleLike(item)">
                            <v-icon v-if="item.liked_by_user" start color="pink" icon="mdi-heart"
                              class="heartBeat"></v-icon>
                            <v-icon v-else start icon="mdi-heart-outline"></v-icon>
                            Like({{ item.likes }})
                            <v-tooltip activator="parent" location="bottom" class=""
                              :text="item.liked_by_user ? 'Liked' : 'Like'"></v-tooltip>
                          </v-btn>
                          <v-spacer></v-spacer>
                          <v-tooltip location="bottom" text="Download">
                            <template v-slot:activator="{ props }">
                              <v-btn v-bind="props" icon="mdi-download" @click="downloadPhoto(item)">
                              </v-btn>
                            </template>
                          </v-tooltip>
              ...
              <v-window-item value="collections">
                <v-card min-height="80vh" class="pa-5 d-flex align-center justify-center"
                ...
                <v-card min-height="80vh" class="pa-5" v-else>
                  ...
                  <v-row>
                    <v-col cols="12" lg="6" xl="4" v-for="item in collectionData.collections" :key="item.id">
                      <v-card class="info-card collection-card d-flex mt-5" color="secondary-lighten-1">
                        <v-img max-width="200" aspect-ratio="1" cover :src="item.cover_photo.urls.small"
                          :lazy-src="item.cover_photo.urls.small" @click="
                            router.push({
                  ...
                  <v-btn v-if="collectionParams.page < collectionData.totalPages" color="" class="gradient info mt-5"
                    block size="large" @click="moreCollections">More Collections...</v-btn>
                </v-card>
              </v-window-item>
